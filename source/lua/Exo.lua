...
function Exo:GetCanEject()
    return self:GetIsPlaying() and not self.ejecting and self:GetIsOnGround() and not self:GetIsOnEntity() and self.creationTime + kExoEjectDuration < Shared.GetTime() and #GetEntitiesForTeamWithinRange("CommandStation", self:GetTeamNumber(), self:GetOrigin(), 4) == 1
end
...
function Exo:HandleButtons(input)
    ...
    if self.ejecting or self.creationTime + kExoEjectDuration > Shared.GetTime() then
        ...
        input.commands = bit.band(input.commands, bit.bnot(bit.bor(Move.Use, Move.Buy, Move.Jump,
                                                                   ...
                                                                   Move.Weapon3, Move.Weapon4, Move.Weapon5, Move.Weapon6, Move.Weapon7, Move.Weapon8, Move.Weapon9, Move.Crouch, Move.Drop, Move.MovementModifier)))
