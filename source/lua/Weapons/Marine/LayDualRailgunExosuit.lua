...
class 'LayDualRailgunExosuit' (Weapon)

LayDualRailgunExosuit.kMapName = "laydualrailgunexosuit"
...
function LayDualRailgunExosuit:OnCreate()
...
function LayDualRailgunExosuit:OnInitialized()
...
function LayDualRailgunExosuit:GetIsValidRecipient(recipient)

    if self:GetParent() == nil and recipient and not GetIsVortexed(recipient) and recipient:isa("Marine") then
    
        local LayDualRailgunExosuit = recipient:GetWeapon(LayDualRailgunExosuit.kMapName)
        return LayDualRailgunExosuit == nil
...
function LayDualRailgunExosuit:GetDropStructureId()
    return kTechId.DualRailgunExosuit
end

function LayDualRailgunExosuit:GetMinesLeft()
    return self.minesLeft
end

function LayDualRailgunExosuit:GetViewModelName()
    return kViewModelName
end

function LayDualRailgunExosuit:GetAnimationGraphName()
    return kAnimationGraph
end

function LayDualRailgunExosuit:GetSuffixName()
    return "dualrailgunexosuit"
end

function LayDualRailgunExosuit:GetDropClassName()
    return "DualRailgunExosuit"
end

function LayDualRailgunExosuit:GetDropMapName()
    return DualRailgunExosuit.kMapName
end

function LayDualRailgunExosuit:GetHUDSlot()
    return 7
end

function LayDualRailgunExosuit:OnTag(tagName)

    PROFILE("LayDualRailgunExosuit:OnTag")
...
function LayDualRailgunExosuit:OnPrimaryAttackEnd(player)
    self.droppingMine = false
end

function LayDualRailgunExosuit:GetIsDroppable()
    return false
end

function LayDualRailgunExosuit:OnPrimaryAttack(player)
...
function LayDualRailgunExosuit:Refill(amount)
    self.minesLeft = amount
end

function LayDualRailgunExosuit:PerformPrimaryAttack(player)
...
function LayDualRailgunExosuit:OnHolster(player, previousWeaponMapName)
...
function LayDualRailgunExosuit:OnDraw(player, previousWeaponMapName)
...
function LayDualRailgunExosuit:Dropped(prevOwner)
...
function LayDualRailgunExosuit:GetPositionForStructure(player)
...
function LayDualRailgunExosuit:GetGhostModelName()
    return LookupTechData(self:GetDropStructureId(), kTechDataModel)
end

function LayDualRailgunExosuit:OnUpdateAnimationInput(modelMixin)

    PROFILE("LayDualRailgunExosuit:OnUpdateAnimationInput")
...
if Client then
    ...
    function LayDualRailgunExosuit:OnProcessIntermediate(input)
...
function LayDualRailgunExosuit:GetShowGhostModel()
    return self.showGhost
end

function LayDualRailgunExosuit:GetGhostModelCoords()
    return self.ghostCoords
end   

function LayDualRailgunExosuit:GetIsPlacementValid()
    return self.placementValid
end

Shared.LinkClassToMap("LayDualRailgunExosuit", LayDualRailgunExosuit.kMapName, networkVars)
